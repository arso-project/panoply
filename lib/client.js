...
class Client {
  ...
  async put (record) {
    ...
    const batchStream = this.ws('batch', { encoding: ndjson })
    const putRequest = streamToAsyncRequest(batchStream)
    await putRequest({
      op: 'put',
      ...record
    })
...
function streamToAsyncRequest (stream) {
  return function (msg) {
    let received = false
    return new Promise((resolve, reject) => {
      stream.write(msg)
      stream.end()
      stream.on('data', data => {
        if (received) stream.emit('error', new Error('Write after end'))
        received = true
        resolve(data)
      })
...
function streamToAsyncResponse (stream, asyncFn) {
  let received = false
  stream.on('data', data => {
    if (received) stream.emit('error', new Error('Write after end'))
    received = true
