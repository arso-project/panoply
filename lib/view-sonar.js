...
const log = require('../lib/log').child({ component: 'sonar' })
...
function sonarView (db, cstore, opts) {
  ...
  return {
    ...
    map (msgs, next) {
      ...
      _map(msgs)
        .then(res => {
          log.debug('indexed %d [time: %s]', msgs.length, time())
          next(null, res)
        })
        .catch(err => {
          log.error(err)
      ...
      query (kcore, args, cb) {
        ...
        _query(string)
          .then(response => {
            ...
            stream.push(null)
            log.debug('query "%s": %d results [time: %d]ms', string, results.length, time())
          })
